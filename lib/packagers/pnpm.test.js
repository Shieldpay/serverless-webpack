...
describe('pnpm', () => {
  ...
  describe('getProdDependencies', () => {
    it('should use pnpm ls', () => {
      Utils.spawnProcess.returns(BbPromise.resolve({ stdout: '{}', stderr: '' }));
      return expect(pnpmModule.getProdDependencies('myPath', 1)).to.be.fulfilled.then(result => {
    ...
    it('should default to depth 1', () => {
      Utils.spawnProcess.returns(BbPromise.resolve({ stdout: '{}', stderr: '' }));
      return expect(pnpmModule.getProdDependencies('myPath')).to.be.fulfilled.then(result => {
        expect(result).to.be.an('object').that.is.empty;
        expect(Utils.spawnProcess).to.have.been.calledOnce,
        expect(Utils.spawnProcess.firstCall).to.have.been.calledWith(sinon.match(/^pnpm/), [
          'list',
          '--depth=1',
          '--parseable',
          '--production'
    ...
    it('should reject on critical pnpm errors', () => {
      Utils.spawnProcess.returns(
        BbPromise.reject(new Utils.SpawnError('Exited with code 1', '', 'pnpm failed.\nerror Could not find module.'))
      );
  ...
  describe('install', () => {
    it('should use pnpm install', () => {
      ...
      return expect(pnpmModule.install('myPath', {})).to.be.fulfilled.then(result => {
        expect(result).to.be.undefined;
        expect(Utils.spawnProcess).to.have.been.calledOnce;
        expect(Utils.spawnProcess).to.have.been.calledWithExactly(sinon.match(/^pnpm/), ['install'], {
          cwd: 'myPath'
        });
        return null;
      });
    ...
    it('should use ignoreScripts option', () => {
      Utils.spawnProcess.returns(BbPromise.resolve({ stdout: 'installed successfully', stderr: '' }));
      return expect(pnpmModule.install('myPath', { ignoreScripts: true })).to.be.fulfilled.then(result => {
        expect(result).to.be.undefined;
        expect(Utils.spawnProcess).to.have.been.calledOnce;
  ...
  describe('prune', () => {
    ...
    it('should call install', () => {
      return expect(pnpmModule.prune('myPath', {})).to.be.fulfilled.then(() => {
  ...
  describe('runScripts', () => {
    it('should use pnpm run for the given scripts', () => {
      ...
      return expect(pnpmModule.runScripts('myPath', [ 's1', 's2' ])).to.be.fulfilled.then(result => {
        expect(result).to.be.undefined;
        expect(Utils.spawnProcess).to.have.been.calledTwice;
        expect(Utils.spawnProcess.firstCall).to.have.been.calledWithExactly(sinon.match(/^pnpm/), [ 'run', 's1' ], {
          cwd: 'myPath'
        });
        expect(Utils.spawnProcess.secondCall).to.have.been.calledWithExactly(sinon.match(/^pnpm/), [ 'run', 's2' ], {
          cwd: 'myPath'
        });
        return null;
      });
