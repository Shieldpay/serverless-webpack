...
describe('pnpm', () => {
  ...
  describe('getProdDependencies', () => {
    it('should use pnpm list', () => {
      ...
      Utils.spawnProcess.returns(BbPromise.resolve({ stdout: JSON.parse('{}'), stderr: '' }));
    ...
    it('should transform pnpm trees to npm dependencies', () => {
      ...
      Utils.spawnProcess.returns(BbPromise.resolve({ stdout: JSON.parse(testPnpmResult), stderr: '' }));
      return expect(pnpmModule.getProdDependencies('myPath', 1)).to.be.fulfilled.then(result => {
        expect(result).to.deep.equal(expectedResult);
        return null;
      });
